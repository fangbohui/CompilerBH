...
import CFG.Instruction.ComputingInstruction.BinaryInstruction.OtherBinaryInstructions.LessEqualInstruction;
...
public class SmallerEqualExpression extends BinaryExpression {
	private SmallerEqualExpression(Type type, Boolean isLV, Expression leftExpression, Expression rightExpression) {
	...
	public static Expression getExpression(Expression leftExpression, Expression rightExpression) {
		if (leftExpression.type instanceof IntType && rightExpression.type instanceof IntType) {
			if (leftExpression instanceof IntConstant && rightExpression instanceof IntConstant) {
				...
				return new SmallerEqualExpression(BoolType.getType(), false, leftExpression, rightExpression);
			}
			...
			if (leftExpression instanceof StringConstant && rightExpression instanceof StringConstant) {
				...
				return FunctionCallExpression.getExpression((Function) Environment.symbolTable.get("FBH_string_le").type, parameters);
			}
		}
	...
	public void emit(ArrayList<Instruction> instructions) {
		...
		instructions.add(LessEqualInstruction.getInstruction((VirtualRegister) operand, leftExpression.operand, rightExpression.operand));
	}
}
